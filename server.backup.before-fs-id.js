...
if ((process.env.REPORTS_BACKEND || "mongo").toLowerCase() === "fs") {
  ...
  // Save/update ONE report (shape matches your existing FS route)
  app.post("/api/report", async (req, res) => {
    try {
      ...
      console.error("FS /api/report (POST) error:", e);
      return res.status(500).json({ ok:false, error:String(e) });
    }
  });

  // Get ONE report (returns the doc directly, like your old route)
  app.get("/api/report", async (req, res) => {
    try {
      ...
      console.error("FS /api/report (GET) error:", e);
      return res.status(500).json({ ok:false, error:String(e) });
    }
  });

  // List reports (returns an array, like your old route)
  app.get("/api/reports", async (req, res) => {
    try {
      const list = await __fsStore.listReports(__filt(req.query));
      return res.json(list);
    } catch (e) {
      console.error("FS /api/reports (GET) error:", e);
      return res.status(500).json({ ok:false, error:String(e) });
    }
  });
}
...
(async () => {
  ...
  try {
    ...
    try {
      ...
      app.get("/api/debug/memdb", (_req, res) =>
        res.json({ memory: true, uri: memUri, binaryVersion: "6.0.6" })
      );
    } catch (e2) {
...
app.post("/api/reports", async (req, res) => {
  try {
    ...
    const existing = merge ? await Report.findOne(filter).lean() : null;

    let answers84Obj;
    if (merge && existing?.answers) {
      answers84Obj = cloneAnswers84(existing.answers);
      const changes = normalizeAnswerChanges(req.body.answers || {});
      Object.assign(answers84Obj, changes);
    } else {
      answers84Obj = cloneAnswers84(req.body.answers || {});
    }
    ...
    if (DEBUG_API) console.log("POST /api/reports upserted:", doc?._id?.toString(), institution, month, year, { merge });
