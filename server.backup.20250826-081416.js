...
/* ——— Space dump of ALL reports ——— */
app.get("/api/reports/space/all", async (req, res) => {
  try {
    const wantFull = String(req.query.full || "").toLowerCase();
    const projection = (wantFull === "1" || wantFull === "true")
      ? "+answers +cumulative +eyeBank +visionCenter"
      : "district institution month year updatedAt";
    const docs = await Report.find({}).select(projection).lean();
    res.json({ ok: true, count: docs.length, reports: docs });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
});
